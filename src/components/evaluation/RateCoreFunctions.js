...
export default function RateCoreFunctions({ response, open }) {
  ...
  const coreFunc = coreFunctions?.filter(
    (coreFunc) => coreFunc?.id === funcId
  )[0];

  const successIndicator = coreFunc?.successIndicators?.filter(
    (successIndicator) => successIndicator?.id === indicatorId
  )[0];

  const handleSubmit = (values, { resetForm }) => {
    dispatch(rateCoreFunctions({ ...values, responseId, funcId, indicatorId }));
