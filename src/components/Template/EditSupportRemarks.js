...
export default function EditSupportRemarks({ response, open }) {
  ...
  const cf = supportFunctions?.filter((cf) => cf?.id === funcId)[0];
  ...
  const handleDelete = () => {
    dispatch(deleteSupportRemarks({ responseId, funcId, succId: indicatorId }));
    return open(false);
  };
